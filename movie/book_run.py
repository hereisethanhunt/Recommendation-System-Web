...
matrix = [[-1 for _ in range(5000)] for _ in range(3610)]
avg_item_rating = []
avg_user_rating = []
...
def read():
	...
	data = []
	with open("bookimp.txt" , "r") as file:
		for line in file :
			n = line.split('|')
			print n
			d = {}
...
def IICF(user , item , neighborCount):
	...
	if c==0:
		d.append( (matrix[user][item],item) )
	d.sort(reverse = True)
	# print d
...
def test(neighborCount):
	...
	for line in input_file:
		...
		rating =  abs(IICF(x , y , neighborCount))
		# print str(matrix[x][y]) +" "+ str(rating)
		if rating >= 0:
			counter += 1
			r = int(rating)
			if rating - r >= 0.5:
				r += 1
			if abs(r- matrix[x][y])<=2 :
				correct += 1
				output_file.write("0\n")
			else:
				output_file.write("1\n")
